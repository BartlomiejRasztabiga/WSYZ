...
var weekly_transport_to_stores {WEEK,WAREHOUSES,STORES,VEGETABLES} >= 0;		# tons transported from warehouses to stores weekly
...
minimize Total_Cost:
	...
	+
	sum {we in WEEK, w in WAREHOUSES, s in STORES, v in VEGETABLES}
   		distance_to_store[w,s] * km_cost * weekly_transport_to_stores[we,w,s,v];
    
# ograniczenie: transport do sklepów + należy zachować minimalne zapasy każdego z warzyw  np. 10% średniej sprzedaży w tygodniu
subject to Store_Weekly_Supply {we in WEEK, v in VEGETABLES, s in STORES}:
	sum {w in WAREHOUSES}
		weekly_transport_to_stores[we, w, s, v] = 1.1 * weekly_sales_forecast1[s, we, v];
	
# TODO obslugiwac transport co tydzien, a nie co rok * 52 :p
subject to Warehouse_Supply {w in WAREHOUSES, v in VEGETABLES}:
	sum {p in PRODUCENTS} yearly_transport_to_warehouses[p, w, v] >= sum {we in WEEK, s in STORES} weekly_transport_to_stores[we, w, s, v];
...
subject to Store_Warehouse_Max_Capacity {we in WEEK, s in STORES}:
	sum {w in WAREHOUSES, v in VEGETABLES} weekly_transport_to_stores[we, w, s, v] <= store_warehouse_capacity[s];